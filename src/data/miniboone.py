...
def load_miniboone_data(config):
    df = pd.read_csv(Path(config['data_dir']) / "miniboone.csv")

    features = ['Feature 1', 'Feature 17', 'Feature 23', 'Feature 32', 'Feature 3', 'Feature 27', 'Feature 12',
                'Feature 4', 'Feature 25', 'Feature 2', 'Outcome']
    ...
    x = torch.tensor(df.drop(columns=['Outcome']).values, dtype=torch.float32)
